/**
 * @file include/submodels/latent_class/transformed_parameters/fit.stan
 ...
 */
simplex[n_classes] mix_probability; // baseline class probabilities
matrix[n_classes, K_mix] mix_location; // ordered and unrestricted class locations
vector[P_class_subject] mix_class_coefficient_subject = rep_vector(0, P_class_subject); // subject-domain formulaClass coefficients on their multinomial-logit scale; when P_class_subject=0 this is a valid empty vector and no draw column is emitted
vector[P_class_marker] mix_class_coefficient_marker = rep_vector(0, P_class_marker); // marker-domain formulaClass coefficients on their multinomial-logit scale; when P_class_marker=0 this is a valid empty vector and no draw column is emitted
if (mix_ordering == 2) {
