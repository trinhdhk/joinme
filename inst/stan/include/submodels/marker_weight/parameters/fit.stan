 ...
 * name requests moving Student-t degrees of freedom, all active sets share
 * one positive excess above two. Optional regularised-horseshoe auxiliaries are confined to
 * this submodel.
 */
  ...
  vector<lower=0>[prior_marker_weight_family == 1 && estimate_marker_weight_df == 1 && n_marker_weight_means > 0 ? 1 : 0] marker_weight_df_excess; // single learned nu minus two shared by every active Student-t marker-weight set; positivity guarantees finite conditional variance
