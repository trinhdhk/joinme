...
parameterisation \alpha = z\_{\alpha} \cdot sd\_{\alpha}.
...
latent perturbation z\_{im} and a non-negative loading \lambda_m, so the
identified form is \eta\_{im} = \alpha_m + x_i^\top \beta_m + \lambda_m
z\_{im} with \lambda_m \ge 0 and z\_{im} \sim \mathcal{N}(0, 1).
...
all internal time calculations are performed on a scaled domain t' \in
\[0, 1\]:

t' = \frac{t}{t\_{max}}, \quad t\_{max} = \max_i(T_i)
...
time-varying random effects \eta\_{id}(s), this integral has no closed
form.
...
\int_0^{T_i} h_i(s) ds \approx \frac{T_i}{2} \sum\_{j=1}^{K} w_j h_i
\left( \frac{T_i}{2}(x_j + 1) \right)
...
- **Functional bytecode**: a small translation program supporting basic
  ...
  trigonometric and hyperbolic functions, power functions, the standard
  normal CDF (`Phi`, `pnorm`), and the standard normal quantile
  (`inv_Phi`, `qnorm`, `probit`).
- **Monotone I-spline**: smooth monotone mapping with user-supplied
  knots and coefficients.
- **Ordered piecewise linear**: fixed knots with posterior monotone
  ordinates reconstructed from cumulative simplex increments.
...
Dynamic predictions \pi_i(u\|t) are efficiently computed using the same
Stan code structure (via `generated quantities` or a separate
...
dynamic prediction use the same posterior coefficient vector. The fit
samples a (K-1) simplex, cumulatively sums its positive elements,
applies the requested sign, and anchors the first ordinate at zero.
Dynamic prediction receives the resulting draw-specific ordinates
directly; it never reconstructs the curve from a user-supplied response
vector.
