...
The examples use the current user-facing posterior interface:

- `draws()` for renamed posterior extraction,
- `mcmc_plot()` for bayesplot-backed MCMC displays,
- explicit plot wrappers such as `longitudinal_plot()` and `survival_plot()`,
- compatibility class aliases `JoiNMeFit` and `PredJoiNMeFit` in addition to
