...
Prediction enforces marker-level consistency: `newdataLong$marker` must use only levels seen during fitting. Internally, dynamic prediction outputs are stabilized by averaging over dynpred posterior rows for each indexed draw before constructing quantile summaries.
...
# - type = "ispline_penalized": provide x and y training pairs, and lambda
#   controls smoothness (larger lambda -> smoother transform).
```
