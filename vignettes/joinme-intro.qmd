...
- Variance association (CORR) derived from the marker-by-subject
...
```{r intro-plot}
plot(pred, which = c("longitudinal", "survival"), scale = "epred", combined = TRUE)
```
...
`ranef(fit)` and `corr(fit)` are nested by `formulaLong` and `formulaDist`.
For prediction objects, `ranef(pred)` and `corr(pred)` are available only when
