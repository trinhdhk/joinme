...
`ranef(fit)`, `corr(fit)`, and `vcov(fit)` are nested by `formulaLong` and `formulaDist`.
For prediction objects, `ranef(pred)`, `corr(pred)`, and `vcov(pred)` are available only when
