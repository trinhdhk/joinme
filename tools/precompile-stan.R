...
is_wsl_mode <- function() {
    if (!isTRUE(.Platform$OS.type == "windows")) return(FALSE)
    cmdstan_path <- tryCatch(cmdstanr::cmdstan_path(), error = function(e) "")
    grepl("//wsl$/", tolower(cmdstan_path), fixed = TRUE) || Sys.getenv("CMDSTANR_USE_WSL") == "1"
}
...
clean_cache_dir <- function(cache_dir) {
    ...
    cached_files <- list.files(cache_dir, full.names = TRUE, all.files = TRUE, no.. = TRUE)
...
if (quiet_require("cmdstanr")) {
    ...
    if (!is.na(ver)) {
        ...
        for (sf in stan_files) {
            ...
            exe_file <- cached_exe_path(cache_dir = cache_dir, model_base_name = model_base_name)
